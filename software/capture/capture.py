...
def addDate(img, time):
    ...
    cv2.putText(img, text, (10, height - 10), cv2.FONT_HERSHEY_SIMPLEX,
                1, (0, 0, 0), 3, lineType=cv2.LINE_AA)
    cv2.putText(img, text, (10, height - 10), cv2.FONT_HERSHEY_SIMPLEX,
                1, (255, 255, 255), lineType=cv2.LINE_AA)


arg_parser = argparse.ArgumentParser(
    description='Show and capture espcam and webcam')
arg_parser.add_argument('-e', dest='espcam',
                        action='store_true', help='record espcam')
arg_parser.add_argument('-w', dest='webcam',
                        action='store_true', help='record webcam')
arg_parser.add_argument('-s', '--sleep_ms', type=int,
                        default=1000, help='sleep time (ms) between captures')
...
if args.webcam:
    # Use 0 for the default camera, 2 for USB webcam
    webcam = cv2.VideoCapture(2)
    webcam.set(cv2.CAP_PROP_BUFFERSIZE, 1)
    # WTF we need to capture a first frame before setting exposure...
    _, _ = webcam.read()
...
while True:
    ...
    if args.webcam:
        ...
        scaled_frame = cv2.resize(
            webcam_frame, None, fx=0.5, fy=0.5, interpolation=cv2.INTER_LINEAR)
