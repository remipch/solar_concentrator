...
MINI_MOCK_FUNCTION(motors_hw_start_move,
                   void,
                   (panel_t panel, motors_direction_t direction, bool continuous),
                   (panel, direction, continuous));
...
TEST(initialize, []() {
    ...
    motors_state_t state = motors_state_machine_update(
        motors_state_t::UNINITIALIZED, motors_transition_t::NONE, panel_t::NONE, motors_direction_t::NONE);
...
TEST(move, []() {
    ...
    MINI_MOCK_ON_CALL(motors_hw_start_move, [&](panel_t panel, motors_direction_t direction, bool continuous) {
        EXPECT(panel == panel_t::PANEL_A);
        EXPECT(direction == DIRECTION_1);
        EXPECT(!continuous);
    });
    motors_state_t state = motors_state_machine_update(
        motors_state_t::STOPPED, motors_transition_t::START_MOVE_ONE_STEP, panel_t::PANEL_A, DIRECTION_1);
    EXPECT(state == motors_state_t::MOVING);

    // Change direction while already moving
    MINI_MOCK_ON_CALL(motors_hw_start_move, [&](panel_t panel, motors_direction_t direction, bool continuous) {
        EXPECT(panel == panel_t::PANEL_B);
        EXPECT(direction == DIRECTION_2);
        EXPECT(!continuous);
    });
    state = motors_state_machine_update(
        motors_state_t::MOVING, motors_transition_t::START_MOVE_ONE_STEP, panel_t::PANEL_B, DIRECTION_2);
    EXPECT(state == motors_state_t::MOVING);
    ...
    state = motors_state_machine_update(
        motors_state_t::MOVING, motors_transition_t::START_MOVE_CONTINUOUS, panel_t::PANEL_A, DIRECTION_3);
    EXPECT(state == motors_state_t::MOVING);
    ...
    state = motors_state_machine_update(
        motors_state_t::MOVING, motors_transition_t::NONE, panel_t::NONE, motors_direction_t::NONE);
    EXPECT(state == motors_state_t::MOVING);
    ...
    state = motors_state_machine_update(
        motors_state_t::MOVING, motors_transition_t::STOP, panel_t::NONE, motors_direction_t::NONE);
...
TEST(stopping, []() {
    ...
    MINI_MOCK_ON_CALL(motor_hw_get_state, []() { return motor_hw_state_t::MOVING; });
    motors_state_t state = motors_state_machine_update(
        motors_state_t::STOPPING, motors_transition_t::NONE, panel_t::NONE, motors_direction_t::NONE);
    EXPECT(state == motors_state_t::STOPPING);
